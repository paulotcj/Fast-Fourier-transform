...
class Signal:
    ...
    def __init__(self, duration: float, num_samples: int) -> None:
        self.time_span: np.ndarray  = np.linspace(start=0, stop=duration, num=num_samples)
        self.values: np.ndarray     = self._compose(time_span=self.time_span)
        self.num_samples: int       = self.values.size
        self.step_interval: float   = float(self.time_span[1] - self.time_span[0])  # seconds per sample
        self.sample_rate: float     = 1.0 / self.step_interval                      # samples per second (Hz)

    @staticmethod
    def _compose(time_span: np.ndarray) -> np.ndarray:
...
class DFT:
    ...
    def _compute(self) -> np.ndarray:
        ...
        for k in range(num_samples):
            ...
            for n in range(num_samples):
                ...
                sum_real += self._signal.values[n] * math.cos(x=exp_value)
                sum_imag += self._signal.values[n] * math.sin(x=exp_value)
    ...
    def _compute_magnitude(self) -> np.ndarray:
        ...
        for i in range(self._result.size):
            magnitude[i] = math.sqrt(x=self._result[i].real**2 + self._result[i].imag**2)
        return magnitude
...
class Plotter:
    ...
    def __init__(self, signal: Signal, dft: DFT) -> None:
        self._signal: Signal = signal
        self._dft: DFT       = dft
...
def main() -> None:
    signal: Signal   = Signal(duration=0.875 * 5, num_samples=8 * 5 * 32)
    dft: DFT         = DFT(signal=signal)
